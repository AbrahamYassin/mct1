  ...
  return (
    <div className="max-w-md mx-auto bg-white p-6 rounded-2xl shadow-soft mt-10">
      ...
      <form onSubmit={submit} className="space-y-3">
        <input className="w-full border p-2 rounded-lg" placeholder="Nom complet" value={name} onChange={e=>setName(e.target.value)} />
        ...
        <button className="w-full bg-brand-600 text-white p-2 rounded-lg">Créer le compte</button>
      </form>
      <div className="text-sm mt-3">Déjà inscrit ? <Link href="/login" className="underline">Se connecter</Link></div>
    </div>
  )
}
